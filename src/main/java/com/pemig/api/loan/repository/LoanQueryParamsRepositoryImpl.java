...
public class LoanQueryParamsRepositoryImpl implements LoanQueryParamsRepository {

  private final AuthCheck authCheck;
  @PersistenceContext private EntityManager entityManager;

  @Override
  public List<Loan> findLoansByProvidedFilters(
    ...
    Root<Loan> cardRoot = criteriaQuery.from(Loan.class);
//predicates
  ...
  private List<Predicate> extractPredicatesFromFilterParams(
          Map<String, String> params, CriteriaBuilder cb, Root<Loan> root, User user)
    ...
    if (params.containsKey(STATUS_FILTER_STRING)) {
      query.add(
          cb.equal(root.get("status"), LoanStatus.valueOf(params.get(STATUS_FILTER_STRING))));
    }
