...
/**
 * {@literal final} class whose {@literal static} methods are used by {@link LogUtil} classes to
 * log their messages.
 *
 * @author caleb
 * @see LogUtil
...
public final class MethodLoggingMessages {
   ...
   *
   * @param pointInMethod A {@link LogPoint} enum value signifying entrance to or exit from a method.
   ...
   */
  public static String msg(LogPoint pointInMethod, JoinPoint jp) {
    String args = Arrays.toString(jp.getArgs());
    return ((pointInMethod == LogPoint.BEGIN) ? "Making" : "Completed")
        + " the call "
   ...
   * @return A {@link String} that informs us when we enter or exit a method.
   */
  public static String msgWithoutLastArgument(LogPoint pointInMethod, JoinPoint jp) {
    String args =
        Arrays.toString(
            ArrayUtils.remove(
